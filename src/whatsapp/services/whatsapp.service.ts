...
import { ProxyRaw, RabbitmqRaw, SettingsRaw } from '../models';
...
export class WAStartupService {
    ...
    if (this.localRabbitmq.enabled) {
      ...
      if (amqp) {
        if (Array.isArray(rabbitmqLocal) && rabbitmqLocal.includes(we)) {
          const exchangeName = this.instanceName ?? 'xch_whatsapp_api';
  ...
  public async connectToWhatsapp(number?: string): Promise<WASocket> {
    ...
    try {
      ...
      const socketConfig: UserFacingSocketConfig = {
        ...
        browser: number ? [session.CLIENT, session.NAME, release()] : browser,
        version,
  ...
  public async reloadConnection(): Promise<WASocket> {
    try {
      ...
      const socketConfig: UserFacingSocketConfig = {
        ...
        logger: P({ level: this.logBaileys }),
        printQRInTerminal: false,
        browser: this.phoneNumber ? [session.CLIENT, session.NAME, release()] : browser,
        version,
        markOnlineOnConnect: this.localSettings.always_online,
        retryRequestDelayMs: 10,
        connectTimeoutMs: 60_000,
    ...
    try {
      ...
      const messageSent = await (async () => {
        ...
        if (message['conversation']) {
          this.logger.verbose('Sending conversation message');
          return await this.client.sendMessage(
            sender,
            {
  ...
  private async convertToWebP(image: string, number: string) {
    try {
      ...
      this.logger.error('Error converting image to WebP: ' + error);
    }
  }
